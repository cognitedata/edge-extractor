...
func (dr *DigestRequest) GetNewDigestAuthHeader() (header string, err error) {
	var req *http.Request
	...
	if resp.StatusCode == 401 {
		return dr.GetNewDigestAuthHeaderFromResponse(resp)
	}

	return "", nil
}

func (dr *DigestRequest) GetNewDigestAuthHeaderFromResponse(resp *http.Response) (header string, err error) {
	var (
		auth     *Authorization
		wa       *wwwAuthenticate
		waString string
